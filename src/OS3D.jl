...
# Include ICP code (runs in-process with threads)
include("icp/icp.jl")
...
function start_server(; port::Int=8000, open::Bool=true)
    ...
    @info "OS3D running at http://127.0.0.1:$port — press Ctrl+C to stop"
...
function julia_main()::Cint
    try
        ...
        @error "OS3D fatal error" exception=(e, catch_backtrace())
        return 1
    end
    return 0
end
